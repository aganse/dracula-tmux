...
main()
{
  ...
  # Status left
  if $show_powerline; then
    tmux set-option -g status-left "#[bg=${green},fg=${dark_gray}]#{?client_prefix,#[bg=${cyan}],} ${left_icon} #[fg=${green},bg=${gray}]#{?client_prefix,#[fg=${cyan}],}${left_sep}"
    powerbg=${gray}
  else
    tmux set-option -g status-left "#[bg=${green},fg=${dark_gray}]#{?client_prefix,#[bg=${cyan}],} ${left_icon}"
  fi
